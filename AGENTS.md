...
- When reviewing a launch/override path that escalates scope, internal authorization, or admin dispatch, require the PR to gate the escalation on the user-explicit trigger (e.g. `modelOverride?.trim()`, an explicit CLI flag, a non-empty tool param) — not on whatever the planner happens to resolve. Findings of the shape "default spawns receive override authorization" or "in-process dispatch forces a synthetic admin client" are P1 merge blockers when the gate is on the planner output instead of the user input; cite the user-input parameter explicitly in `bestSolution`. Cross-check sibling surfaces that share the same dispatch seam (e.g. `subagent-spawn`, `agent-call`, plugin sub-agent override forwarding) and require a one-sentence explanation of why they are unaffected, or explicit follow-up work.
...
- **Explicit user action vs. resolved/default behavior are different authorization surfaces.** When fixing bugs that span a launch/override path, gate the _expanded_ behavior (admin scope, override authorization, wider dispatch seams) on the _user-explicit_ trigger condition, not on whatever the planner happens to resolve. A planner that always returns a value (default model, persisted override, inherited config) is **not** proof the user asked for an override. Anchor the gate to the user-supplied parameter (`modelOverride?.trim()` from the tool call, not `resolvedModel` from the planner). Otherwise, default/configured/inherited spawns silently receive admin/internal authorization they were never entitled to. When reviewing PRs, ClawSweeper specifically calls this out as `[medium] Default spawns can receive override authorization` and `[medium] In-process explicit overrides force a synthetic admin client`; the same shape recurs across `subagent-spawn`, `agent-call`, `dispatchGatewayMethodInProcess`, and any future tool surface that forwards user params to a privileged gateway method. When authoring the fix, write the spec's `证明可行性评估` section with the explicit/implicit distinction called out before the trigger condition.
