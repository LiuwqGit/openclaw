...
<Note>
**Skills**: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, OpenClaw mirrors eligible skills into the sandbox workspace (`.../skills`) as read-only instruction roots; other private workspace files remain writable. With `"rw"`, workspace skills are readable from `/workspace/skills`, and eligible managed, bundled, or plugin skills are materialized into a generated read-only path. The Docker and Podman backends mount it directly at `<workdir>/.openclaw-skills/skills` (with the default `workdir` of `/workspace`, that is `/workspace/.openclaw-skills/skills`); the SSH and OpenShell backends keep the nested layout `<workdir>/.openclaw/sandbox-skills/skills`.
