 ...
 * multi-agent store resolution; this command only validates, forwards, and
 * formats results, and propagates a non-zero exit on transport or RPC failures
 * so automation never mistakes a failed lookup for "no matches".
 */
...
export async function sessionsSearchCommand(
  ...
  const rpcOpts = {
    ...
    // Search is a bounded read; keep the explicit override but default to the
    // shared client timeout instead of an ambiguous open-ended wait.
  ...
  try {
    result = (await callGatewayFromCliWithTransport("sessions.search", rpcOpts, params, {
      defaultTimeoutMs: 15_000,
    })) as SessionsSearchResult;
  } catch (err) {
